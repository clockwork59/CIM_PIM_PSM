...
IFC_FILES = {
    "NBU_HVAC":        BASE / "docs/NBU_MedicalClinic/NBU_MedicalClinic_Eng-HVAC.ifc",
    "NBU_ARCH":        BASE / "docs/NBU_MedicalClinic/NBU_MedicalClinic_Arch.ifc",
    "NBU_MEP":         BASE / "docs/NBU_MedicalClinic/NBU_MedicalClinic_Eng-MEP-Optimized.ifc",
    "NBU_ELE":         BASE / "docs/NBU_MedicalClinic/NBU_MedicalClinic_Eng-ELE.ifc",
    "NVW_L200_HVAC":   BASE / "docs/NVW_DCR-LOD/NVW_DCR-LOD200_Eng-HVAC.ifc",
    "NVW_L300_HVAC":   BASE / "docs/NVW_DCR-LOD/NVW_DCR-LOD300_Eng-HVAC.ifc",
}
...
def extract_inventory(ifc_path: Path) -> dict:
    ...
    for etype in FUNCTIONAL_TYPES:
        ...
        if entities:
            ...
            for e in entities[:5]:
                ...
                samples.append({"id": e.GlobalId, "name": name, "tag": tag})
            result["equipment_sample"][etype] = samples
    ...
    for space in model.by_type("IfcSpace")[:50]:
    ...
    try:
        ...
        pass
    result["psets_found"] = sorted(psets)
    return result
...
if __name__ == "__main__":
    ...
    for key, inv in inventories.items():
        ...
        print(f"  Pset 集合 ({len(psets)}): {', '.join(psets[:10])}{'...' if len(psets)>10 else ''}")
