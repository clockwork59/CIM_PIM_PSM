...
BAS_INST   = Namespace("http://hospital-cim.org/v4/bas/instances#")
...
ALARM_TARGETS = {
    "t1_primary": {
        "equip_id":   "SD-F5-001",
        "point_name": "SD-F5-001_SMKDET",
        "zone":       "FAS-ZONE-05",
        "delay":      0,    # seconds after t1 reference
    },
    "t1_secondary": {
        "equip_id":   "SD-F5-003",
        "point_name": "SD-F5-003_SMKDET",
        "zone":       "FAS-ZONE-05",
        "delay":      30,   # +30s → triggers dual-zone auto-confirmation
    },
}
...
class SimClock:
    ...
    def inject_alarm(self, snapshot_file: Path, target_key: str) -> bool:
        ...
        if equip_uri is None:
            ...
            equip_uri = URIRef(f"http://hospital-cim.org/v4/instances#{equip_id}")
